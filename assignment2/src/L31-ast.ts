...
// L31
export interface ClassExp {tag: "ClassExp"; args: VarDecl[]; bindings: Binding[]}
...
const unparseClassExp = (ce: ClassExp) : string =>{
    return `(class (${map((p: VarDecl) => p.var, ce.args).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL31(b.val)})`, ce.bindings).join(" ")}))`}
